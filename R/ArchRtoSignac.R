...
ArchR2Signac <- function(
    ...
    if(fragments_fromcellranger == "YES" | fragments_fromcellranger == "Y" | fragments_fromcellranger == "Yes") {
        ...
        cur_chromatin <- Signac::CreateChromatinAssay(
            ...
            #genome=refversion, # need network
    ...
    if(length(seurat_list) > 1){
        ...
        SeuratObject <-RenameCells(seurat_list[[1]],add.cell.id = samples)
    }
...
addDimRed <- function(
    ...
    if(reducedDims == 'Harmony'){
        ...
        harmony_matrix <- harmony_matrix[rownames(ArchRProject),]
        ...
        LSI_matrix <- LSI_matrix[rownames(ArchRProject),]
        ...
        LSI_matrix2 <- LSI_matrix2[rownames(ArchRProject),]
